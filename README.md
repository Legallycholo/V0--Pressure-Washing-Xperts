...
| Database (leads) | [Supabase](https://supabase.com) Postgres (`public.leads`; browser inserts via anon key + RLS) |
...
Quote and contact forms save submissions to Supabase (`public.leads`) using the browser client and Row Level Security (anon insert policy). Copy [`.env.example`](.env.example) to `.env.local` and set:

| Variable | Required | Description |
| --- | --- | --- |
| `NEXT_PUBLIC_SUPABASE_URL` | Yes | Supabase project URL (Settings → API). |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Yes | Supabase anon (public) key for client-side inserts; RLS restricts what anon can do. |
...
Pushes to `main` are intended to deploy on Vercel (as with the original v0-linked workflow). Set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` on the Vercel project.
